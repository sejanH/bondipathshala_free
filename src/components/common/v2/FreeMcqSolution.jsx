...
const FreeMcqSolution = ({ question, counter = 1, type="mcq" }) => {
  ...
  return (
    <div className="mb-8 pb-2 rounded-lg shadow-[0px_0px_6px_2px_rgba(0,0,0,0.5)] relative">
      ...
      <div className="mb-1">
        {question.type === false ? (
          <div className='p-4'>
            <img className="rounded-lg w-full" src={`${process.env.REACT_APP_FILES_HOST}/${type==="mcq-only"? question.question:question.question}`} alt='' />
          </div>
        ):(
          <div className="rounded-lg text-[1.5rem] font-bold p-4"><Latex>{question.question}</Latex></div>
          )}
      </div>
...
export default FreeMcqSolution
