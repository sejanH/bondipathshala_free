      ...
      <div className="modal overflow-y-scroll flex-wrap modal-middle">
        ...
        >
          ...
          {!hideCloseBtn && (
            <label
              htmlFor="my-modal-4"
              className="btn bg-inactive border-0 btn-xs btn-circle absolute right-2 top-2"
            >
              <img className="w-3 h-3" src={cross} alt="x" />
            </label>
          )}
          <div className="relative md:min-h-[18rem] overflow-auto p-4">
            <h2 className="text-3xl font-bold mb-2">
              {result.examName} (Free Exam)
            </h2>
            <div className="grid tab:grid-cols-5 grid-flow-rows gap-x-4 tab-max:grid-cols-2 sm-min:grid-cols-2 gap-y-4 place-content-center resultSummery">
              <div className="resultSummery_col">
                ...
                <span className="resultSummery_col_bottom">
                  {result.totalMarksMcq / result.marksPerMcq}
                </span>
                <div className="text-center text-xl text-slate-700">
                  Full Marks
                  <span className="pl-1 font-bold text-title-2">
                    {result.totalMarksMcq}
                  </span>
                </div>
              </div>
              <div className="resultSummery_col">
                ...
                <span className="resultSummery_col_bottom">
                  {result.totalCorrectAnswer}
                </span>
                <div className="text-center text-xl text-slate-800 font-bold">
                  {result.totalCorrectAnswer} x {result.marksPerMcq}
                </div>
              </div>
              <div className="resultSummery_col">
                ...
                <div className="text-center text-xl text-slate-800 font-bold">
                  ({result.marksPerWrong}) x {result.totalWrongAnswer}
                </div>
              </div>
              <div className="resultSummery_col">
              ...
              <div className="resultSummery_col_circle tab-max:col-span-2">
                <CircularProgressbarWithChildren
                  value={
                    wrongPercentage === 100 ? 0 : result.totalCorrectAnswer
                  }
                ...
                >
                  <div className="marksContainer">
                    {result.totalObtainedMarks === -5000 && 
                    ...
                    styles={buildStyles({
                      rotation:
                        wrongPercentage === 100
                          ? 0
                          : parseFloat(
                              result.totalCorrectAnswer / noOfQuestions
                            ),
            ...
            <div className="text-center p-2 text-white text-xl">
              ...
              <span className="py-1 px-3  bg-[#203a4a] rounded-e-3xl">
                {parseInt(result?.rank) === -1 ? 'Pending' : myRank}
              </span>
            </div>
          </div>
        </div>
