...
const ExamRules = () => {
  ...
  useEffect(() => {
    if (TOKEN) {
      ...
      axios
        .get("/api/exam/examruleget?examId=" + params.get("examId"))
        .then(({ data }) => {
          setExamRules(data);
        })
        .catch((err) => {
  ...
  const doMagic = () => {
    ...
    axios.defaults.headers.common["Authorization"] = "Bearer " + TOKEN;
    axios
      .get("/api/freestudent/startexam?eId=" + params.get("examId"))
      .then(({ data }) => {
        localStorage.setItem(
          `${params.get("examId")}timing`,
      ...
      .catch((err) => {
        ...
        setError({
          message: errMsg,
          cssClass: "error text-white bg-color-one w-[260px]",
          position: "center",
          alignment: "middle",
        });
              ...
              <div className="col-start-2 md:col-start-1 col-span-4 md:col-span-6 headerShadow ">
                <div className="flex btn-theme rounded-t-xl py-1 justify-center">
                  <span className="text-2xl font-bold capitalize text-white">
                    {examDetails?.name}{" "}
                    <span className="md:hidden">(Free Live Exam)</span>
                  </span>
                </div>
                {/* user input box */}
                <div className="p-4  md:py-4 rounded-e-md bg-white">
                  ...
                  <div className="flex flex-row flex-wrap justify-center mb-12 space-x-4">
                    <div className="examSummery_col py-2">
                      <span className="examSummery_col_top">
                        Total Questions
                      </span>
                      <span className="examSummery_col_bottom">
                        {examDetails?.totalQuestionMcq}
                      </span>
                    </div>
                    <div className="examSummery_col py-2">
                      <span className="examSummery_col_top">
                  ...
                  <div className="border border-color-six px-2 py-2 rounded-md">
                    ...
                    {examRules && (
                      <img
                        src={`${process.env.REACT_APP_FILES_HOST}/${examRules?.ruleILink}`}
                      />
                    )}
                  </div>
                  <div className="block">
                    {btnText === "Start Exam" ? (
                      <button
                        type="button"
                        className="w-full text-center btn-hover border-0 text-white rounded-md py-3 mt-6 mb-4"
                        onClick={doMagic}
                      >
                        {btnText}
